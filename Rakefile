...
Rake::Task["release"].enhance do
  ...
  system "open https://github.com/viral810/retry-rb/releases"
end
...
task :verify_gemspec_files do
  ...
  gemspec_files = Gem::Specification.load("retry-rb.gemspec").files.sort
  ignored_by_git = gemspec_files - git_files
  next if ignored_by_git.empty?

  raise <<~ERROR

    The `spec.files` specified in retry-rb.gemspec include the following files
...
namespace :bump do
  ...
  task :ruby do
    ...
    replace_in_file "retry-rb.gemspec", /ruby_version = .*">= (.*)"/ => lowest
